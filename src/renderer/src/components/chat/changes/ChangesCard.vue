<template>
  <!--
    Turn Changes Card: 一轮对话结束后挂在底部的「本轮改了哪些文件」。

    后端无关——codex 的数据来自 file_change 活动，claude 的来自本轮 Edit/Write
    工具调用（见 lib/review.ts），到这里都已经是 ReviewFile[]。
  -->
  <div class="mt-5 overflow-hidden rounded-xl border border-border/70 bg-muted/20">
    ...
    -->
    <div
      class="flex cursor-pointer items-center gap-2 px-3 py-2.5 text-[length:var(--chat-text-base)]"
      role="button"
      :aria-expanded="open"
    ...
    >
      ...
      <span class="font-mono text-[length:var(--chat-text-d1)] tabular-nums">
      ...
      <button
        type="button"
        class="cursor-pointer rounded-md border border-border px-2.5 py-1 text-[length:var(--chat-text-d1)] hover:bg-muted"
        @click.stop="review"
      >
        审核
      </button>
    ...
    <div v-if="open" class="border-t border-border/60">
      ...
      <button
        ...
        :key="file.path"
        type="button"
        class="flex w-full cursor-pointer items-center gap-2 px-3 py-2 text-left text-[length:var(--chat-text-d1)] hover:bg-muted/40"
...
const props = defineProps<{ files: ReviewFile[]; stats: DiffStats }>()
