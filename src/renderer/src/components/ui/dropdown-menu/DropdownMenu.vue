<template>
  ...
  <div ref="rootEl" class="relative inline-block">
    <button
      ...
      :title="title"
      :disabled="disabled"
      class="flex items-center gap-1 px-2 py-1 rounded text-xs transition-colors bg-secondary text-secondary-foreground hover:bg-accent hover:text-accent-foreground disabled:opacity-50 disabled:cursor-not-allowed focus:outline-none focus-visible:ring-1 focus-visible:ring-ring"
    ...
    >
      <button
        ...
        :title="opt.title"
        class="w-full flex items-center gap-2 px-2 py-1.5 rounded text-xs text-left transition-colors text-popover-foreground hover:bg-accent hover:text-accent-foreground disabled:opacity-40 disabled:cursor-not-allowed"
        :class="opt.value === modelValue ? 'bg-accent/60 text-accent-foreground' : ''"
        @click="onSelect(opt.value)"
      >
        <CheckIcon v-if="opt.value === modelValue" class="w-3 h-3 flex-shrink-0 text-foreground" />
