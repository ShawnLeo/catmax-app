...
## Claude Settings Profiles (`src/main/service/claude-settings-profiles.ts`)

The `claude.catmaxSettings` override layer is the one config file a user can keep **several** of and switch between — the Claude-side equivalent of the bridge's multi-provider library. Three things about it are load-bearing:

- **The descriptor's path is no longer a constant.** `resolveBackendConfigPath()` special-cases `multiProfile: true` and returns `<userData>/backend-settings/claude-profiles/<profile id>.json`, so read/write/validate/reveal, `claudeOverrideSettingsPath()`, the adapter's `Options.settings` injection, and warmup all follow the current profile without knowing profiles exist. Anything new that needs "the active override file" must go through that resolver, never through `descriptor.relativePath` — that field now only names the pre-profile file, and only so the migration can find it.
- **Profile metadata does not live in `settings.json`, deliberately unlike `BridgeProvider`.** A bridge provider is structured non-secret fields; a profile is a whole JSON file whose `env` block routinely holds `ANTHROPIC_AUTH_TOKEN` (the descriptor is `sensitive`, writes are forced 0600). `settings.json` is 0644, gets backed up, and the renderer reads it wholesale. So content stays in per-profile 0600 files and metadata lives in `claude-profiles/index.json`, which is **the source of truth** — orphan `.json` files in that directory are never adopted, because "profile registered but not yet saved" is a legal state that a directory scan cannot tell apart from an orphan.
- **`currentId: ''` is a real state, not a missing value.** It means "catmax injects nothing, everything falls back to `~/.claude`" — the semantics the pre-profile code got from "the file doesn't exist". Losing it would leave users unable to turn the override layer off. In that state the resolver returns a `_none.json` placeholder that is never created: `writeBackendConfigFile` refuses multi-profile writes with no current profile, and the injection side returns null before ever stat-ing it.

Migration runs once, lazily, the first time `index.json` is missing: the old single `claude-settings.json` becomes profile #1 and the old file is **renamed** to `.migrated` rather than deleted. Internal Beta Login owns a fixed `managed` profile (`INTERNAL_BETA_PROFILE_ID`) — login `ensureInternalBetaProfile()`s and switches to it before its `force: true` write, which is what stops it from clobbering a user-authored profile the way the single-file version did; logout deletes that profile and its file, falling back to the first remaining profile.
...
- `AGENTS.md` — a verbatim copy of this file for non-Claude agents, differing only in its header and in this bullet. It is **generated, not hand-maintained**: after a substantive edit here, re-copy the whole file rather than patching the one section you touched. Hand-patching is what let it drift three whole sections out of date before.
